...
BARFSOL=`grep ^BARFSOL= settings.txt | sed "s/^.*=//"`
BARFTEMPSOL=`grep ^BARFTEMPSOL= settings.txt | sed "s/^.*=//"`
...
printf "BARFSOL                       = '$BARFSOL'\n"
printf "BARFTEMPSOL                   = '$BARFTEMPSOL'\n"
...
`cp $BARFSOL $BARFTEMPSOL`
`perl -pi -e "s/^\/\*.*$//; s/^ \*.*$//; " $BARFTEMPSOL`

# --- Modify addresses ---
`perl -pi -e "s/\(deployedAt \+ 2 days\)/$CURRENTTIMEP1M/" $BARFTEMPSOL`
`perl -pi -e "s/\(deployedAt \+ 7 days\)/$CURRENTTIMEP2M/" $BARFTEMPSOL`
`perl -pi -e "s/\(deployedAt \+ 30 days\)/$CURRENTTIMEP2M/" $BARFTEMPSOL`
`perl -pi -e "s/\(deployedAt \+ 60 days\)/$CURRENTTIMEP2M/" $BARFTEMPSOL`
`perl -pi -e "s/\(deployedAt \+ 90 days\)/$CURRENTTIMEP2M/" $BARFTEMPSOL`
`perl -pi -e "s/\(deployedAt \+ 365 days\)/$CURRENTTIMEP2M/" $BARFTEMPSOL`
`perl -pi -e "s/\(deployedAt \+ 3652 days\)/$CURRENTTIMEP3M/" $BARFTEMPSOL`

# --- Check differences ---
TEST=`diff $BARFSOL $BARFTEMPSOL`
echo "--- Differences ---"
echo "$TEST"

FLATTENEDBARFSOL=`./stripCrLf $BARFTEMPSOL | tr -s ' '`
printf "var barfSource = \'$FLATTENEDBARFSOL\';" > $INCLUDEJS
